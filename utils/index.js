...
export async function login() {
  const loginMsg = await wxp.login();
  const loginResult = await base.login(loginMsg.code);
  if (loginResult.status === RequestCode.CODE.SUCCESS.OK) {
    setCache('token', loginResult.data.token);
  }
}